...
// TODO: Allow for a 24-hour option--already supported by calendar turn
 ...
 if ((levz < 0) && (u.is_wearing(itm_wristwatch)))
   ...
   for (int realy = u.posy - SEEY - ext_y_offset; realy <= u.posy + SEEY + ext_y_offset + 1; realy++) {
    if (scent(realx, realy) != 0) {
     ...
     if (!(isBetween(tempx, -2, 2) && isBetween(tempy, -2, 2))) {
      mvwputch(w, realy + SEEY + ext_y_offset - u.posy,
        realx + SEEX + ext_x_offset - u.posx, c_magenta, '#');
      if (mon_at(realx, realy) != -1)
       mvwputch(w, realy + SEEY + ext_y_offset - u.posy,
         realx + SEEX + ext_x_offset - u.posx, c_white, '?');
...
void game::examine()
{
 ...
 if (m.ter(examx, examy) == t_card_science ||
  ...
  break;
  case 1:
    if (u.has_amount(itm_rope_6, 1)) {
   m.ter(examx, examy) = t_ropefence;
   u.moves -= 100;
   u.use_amount(itm_rope_6, 1);
   add_msg("You loop the rope around the posts, making a simple fence");
  break;
 } else {
   add_msg("You need a 6 foot length of rope!");
  break;
 }
  case 2:
