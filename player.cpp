...
void player::load_info(game *g, std::string data)
{
 ...
 dump >> posx >> posy >> str_cur >> str_max >> dex_cur >> dex_max >>
         ...
         underwater >> can_dodge >> oxygen >> active_mission >> xp_pool >> male >> health;
 in_vehicle = inveh != 0;
...
// Find our best skill
 ...
 // If no xp, there's a 1/4 chance
 while (amount > 0 && (xp_pool >= (1 + sklevel[s]) || one_in(has_trait(PF_FASTLEARNER) ? 3 : 4))) {
  amount -= sklevel[s] + 1;
